...
public class EnquiryForm {
	...
	@FindBy(xpath = "//a[@class='pr_rslt sr nrmtab']")
...
//		if (Commanmethod.isEnabled(datefld) && Commanmethod.isEnabled(monthfld) && Commanmethod.isEnabled(yearfld)) {
//			Commanmethod.type(datefld, date);
//			Commanmethod.type(monthfld, month);
//			Commanmethod.type(yearfld, year);
//		}
...
//		else {
//
//			Commanmethod.logger.log(LogStatus.INFO, "DOB field is not enabled for this provider");
//			// CommonFunction.selectDropdown(countryofresidence, "855");
//		}

		// Commanmethod.isSelected(countryofresidence);
		if(Commanmethod.isElementPresent(citizenship)==true)
		{
		...
		if (Commanmethod.isElementPresent(phoneType)==true && Commanmethod.isElementPresent(countryCode)==true && Commanmethod.isElementPresent(phoneNumber)==true) {
			...
			executor.executeScript("arguments[0].scrollIntoView(true);", prcontinstbtn); 
			...
			executor1.executeScript("arguments[0].scrollIntoView(true);", prcdpod); 
			...
			executor2.executeScript("arguments[0].scrollIntoView(true);", cdenqbtn); 
